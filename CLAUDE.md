...
**Block 2 MCP** (`block2_sdk/agent_loop_mcp.py` + `barista_mcp_server.py`)  
Same agent loop as above, but tools are served via the Model Context Protocol. `barista_mcp_server.py` is a FastMCP server spawned automatically as a subprocess; `agent_loop_mcp.py` discovers tools via `session.list_tools()` and calls them via `session.call_tool()` instead of a local dispatch function.

**Block 4 — Agent design patterns** (`block4_patterns/`)  
Three self-contained patterns that build on the Block 3 foundation. See `Patterns.md` for a full explainer.

| File | Pattern | Key concept |
|------|---------|-------------|
| `option_a_memory.py` | Long-term memory | `PreferenceAgent` reads/writes a JSON file via tools, runs before `OrderAgent` to personalise the session |
...
All agents use `claude-sonnet-4-6`. To upgrade, update the `MODEL` constant at the top of each file.
